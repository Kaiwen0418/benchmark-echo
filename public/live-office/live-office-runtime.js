// @ts-nocheck
// Shared UI language and modal helpers now live in:
// - live-office-i18n.ts
// - live-office-modal.ts
// 检测浏览器是否支持 WebP
...
try {
    localStorage.removeItem('assetPositionOverrides');
}
catch (e) { }
let isMoving = false;
...
function setState(state, detail) {
    fetch('/set_state', {
        ...
        .then((res) => {
        if (!res.ok)
            throw new Error(`set_state failed: ${res.status}`);
        return fetchStatus();
    })
        .catch((e) => {
...
function getCurrentScene() {
    if (!game)
        return null;
    if (game.children && game.add)
        return game;
    if (game.scene && game.scene.scenes && game.scene.scenes.length)
...
function mapAssetPathToSprite(path) {
    ...
    if ((path || '').includes('office_bg_small.webp') && officeBgSprite)
        return officeBgSprite;
...
function drawSelectionBoxOnScene(path) {
    ...
    if (!selectionBoxGraphics)
        selectionBoxGraphics = scene.add.graphics();
    const b = sp.getBounds();
...
function saveAssetPositionOverrides() { }
    ...
    try {
        ...
        try {
            ...
            if (data && data.ok && data.items)
                serverPositions = data.items;
        }
        catch (e) { }
        try {
            const res2 = await fetch('/assets/defaults?t=' + Date.now(), { cache: 'no-store' });
            const data2 = await res2.json();
            if (data2 && data2.ok && data2.items)
                serverDefaults = data2.items;
        }
        catch (e) { }
        const children = getSceneChildren();
        for (const ch of children) {
            ...
            if (!ov) {
                ...
                if (hitKey)
                    ov = serverPositions[hitKey] || serverDefaults[hitKey] || assetPositionOverrides[hitKey];
            }
            if (!ov)
                continue;
            ...
            if (Number.isFinite(x) && Number.isFinite(y)) {
                ch.x = x;
                ch.y = y;
                if (Number.isFinite(sc) && sc > 0 && ch.setScale)
                    ch.setScale(sc);
            }
        }
    }
...
function updateAssetConfirmButtonState() {
    ...
    if (panel)
        panel.classList.toggle('active', can);
...
function guessThumbFrameSize(fullW, fullH, path = '') {
    ...
    for (const [re, fw, fh] of hints) {
        if (re.test(p) && fullW % fw === 0 && fullH % fh === 0)
            return { fw, fh };
    }
...
function tryAnimateAssetThumb(item) {
    ...
    img.src = `/live-office/static/${item.path}?t=${Date.now()}`;
}
...
function setAssetVisible(path, visible) {
    ...
    if (!p)
        return;
    if (visible)
        hiddenAssetPaths.delete(p);
    else
        hiddenAssetPaths.add(p);
...
function toggleAssetVisibility(path, ev) {
    ...
    const out = document.getElementById('asset-upload-result');
    if (out)
        out.textContent = nextVisible ? `✅ 已显示：${p}` : `🙈 已隐藏：${p}`;
...
function renderAssetDrawerList() {
    ...
    const assetRank = (path = '') => {
        ...
        if (idx >= 0)
            return idx; // 0~3: 四个主状态最前
        // 按钮素材最不重要：统一沉到列表末尾
        if (p.includes('/btn-') || p.includes('btn-') || p.includes('button'))
            return 1000;
        if (p.includes('guest_anim_'))
            return 999; // guest 动画靠后
        return 100;
    };
    const rows = baseRows
    ...
    rows.forEach(it => {
        ...
        img.src = `/live-office/static/${it.path}?t=${Date.now()}`;
    });
}
async function refreshAssetDrawerList() {
    ...
    try {
        ...
        renderAssetDrawerList();
        if (out)
            out.textContent = `已加载资产：${assetListData.length} ｜ 场景抓取：${sceneAssetItems.length}`;
        if (selectedPath) {
        ...
        if (out)
            out.textContent = '❌ 资产加载失败，请点“刷新”重试';
    }
}
...
function bindAssetDrawerBackgroundDeselect() {
    if (assetDrawerBackgroundBinded)
        return;
...
async function refreshSceneObjectByAssetPath(path) {
    ...
    const url = `/live-office/static/${path}?t=${Date.now()}`;
    return new Promise((resolve) => {
        try {
...
async function commitAndRefresh() {
    ...
    if (out) {
        if (hasFile)
            out.textContent += ' ｜ ✅ 已上传并刷新';
        else
            out.textContent = '✅ 已确认并刷新';
    }
    // 刷新前关闭侧边栏，行为与地图替换一致
...
function markMoveSuccess(outEl, btnEl = null) {
    if (outEl)
        outEl.textContent = t('moveSuccess');
    if (btnEl)
        setButtonDone(btnEl);
    try {
        setState('idle', t('moveSuccess').replace('✅ ', ''));
    }
    catch (e) { }
}
function setWorkingStatus(detail = '工作中') {
    try {
        setState('writing', detail);
    }
    catch (e) { }
}
...
async function saveGeminiConfigFromUI() {
    ...
    try {
        ...
        if (!data.ok) {
            if (msg)
                msg.textContent = `保存失败：${data.msg || res.status}`;
            return;
        }
        if (msg)
            msg.textContent = '✅ 已保存，可重新点击搬家/中介';
        const box = document.getElementById('asset-gemini-config');
        if (box)
...
async function _startAndPollGeneration(body, out, progressMsg) {
    ...
    if (!data.async || !data.task_id)
        return data; // sync fallback (shouldn't happen)
    // Poll for completion
...
async function generateCustomRpgBackground() {
    ...
    try {
        const data = await _startAndPollGeneration({ prompt, speed_mode: speedMode }, out, '🏘️ 正在按中介方案生成底图');
        if (!data.ok) {
...
async function generateRpgBackground() {
    ...
    try {
        const data = await _startAndPollGeneration({ speed_mode: speedMode }, out, '🧳 正在生成新房间');
        if (!data.ok) {
            _handleGenError(data, out);
            return;
        }
...
async function renderHomeFavorites(force = false) {
    ...
    if (!force && homeFavoritesCache.length > 0 && (now - homeFavoritesLoadedAt) < 30000) {
        ...
        try {
            ...
            const out = document.getElementById('asset-move-result') || document.getElementById('asset-upload-result');
            if (out)
                out.textContent = `❌ 收藏列表加载失败：${e.message || e}`;
        }
    }
...
async function deleteHomeFavorite(id) {
    ...
    if (!id)
        return;
    if (!window.confirm('确定删除这个收藏吗？删除后不可恢复。'))
        return;
    try {
...
async function resetSelectedAssetToDefault() {
    ...
    if (!path) {
        if (out)
            out.textContent = '请先选择一个资产';
        return;
    }
    if (!window.confirm(`⚠️ 确定将 ${path} 重置为默认资产吗？`))
        return;
    try {
        ...
        if (!data.ok) {
            if (out)
                out.textContent = `❌ 重置失败：${data.msg || res.status}`;
            return;
        }
        await refreshSceneObjectByAssetPath(path);
        if (out)
            out.textContent = `✅ 已重置为默认资产：${path}`;
    }
    catch (e) {
        if (out)
            out.textContent = `❌ 重置失败：${e}`;
    }
}
async function restoreSelectedAssetPrev() {
    ...
    if (!window.confirm(`⚠️ 确定将 ${path} 回退到上一版吗？`))
        return;
    try {
...
async function toggleAssetDrawer(force) {
    ...
    if (closeBtn)
        closeBtn.textContent = t('drawerClose');
    if (next) {
        ...
        if (assetDrawerAuthed) {
            ...
            const msg = document.getElementById('asset-auth-msg');
            if (msg)
                msg.textContent = t('authDefaultPassHint');
        }
    }
    else {
        assetManualPanelOpen = false;
...
function renderGuestAgentsInScene() {
            ...
            if (isDemoNika || isDemoMercury) {
                ...
                if (!animIdx || animIdx < 1 || animIdx > 6) {
                    ...
                    for (let i = 0; i < aid.length; i++)
                        hash = (hash * 31 + aid.charCodeAt(i)) >>> 0;
                    animIdx = (hash % 6) + 1;
                }
    ...
    Object.keys(guestSprites).forEach(id => {
        if (!seenIds.has(id)) {
            ...
            if (guestTweens[id]) {
                try {
                    guestTweens[id].move && guestTweens[id].move.stop();
                }
                catch (e) { }
                try {
                    guestTweens[id].name && guestTweens[id].name.stop();
                }
                catch (e) { }
...
function maybeShowGuestBubble(time) {
    if (time - lastGuestBubbleAt < 5200)
        return;
    ...
    return fetch('/agents?t=' + Date.now(), { cache: 'no-store' })
        ...
        try {
            ...
            if (!guestWelcomeInitialized) {
                ...
                const newIds = [];
                currentIds.forEach(id => { if (!lastSeenGuestIds.has(id))
                    newIds.push(id); });
                if (newIds.length > 0) {
...
function preload() {
    ...
    this.load.image('office_bg', '/live-office/static/office_bg_small.webp?v={{VERSION_TIMESTAMP}}');
    this.load.spritesheet('star_idle', '/live-office/static/star-idle-v5.png?v={{VERSION_TIMESTAMP}}', { frameWidth: 256, frameHeight: 256 });
    // Furniture
    this.load.image('sofa_idle', '/live-office/static/sofa-idle-v3.png?v={{VERSION_TIMESTAMP}}');
    this.load.image('sofa_shadow', '/live-office/static/sofa-shadow-v1.png?v={{VERSION_TIMESTAMP}}');
    // Decor
    this.load.spritesheet('plants', '/live-office/static/plants-spritesheet.webp?v={{VERSION_TIMESTAMP}}', { frameWidth: 160, frameHeight: 160 });
    this.load.spritesheet('posters', '/live-office/static/posters-spritesheet.webp?v={{VERSION_TIMESTAMP}}', { frameWidth: 160, frameHeight: 160 });
    this.load.spritesheet('coffee_machine', '/live-office/static/coffee-machine-v3-grid.webp?v={{VERSION_TIMESTAMP}}', { frameWidth: 230, frameHeight: 230 });
    this.load.image('coffee_machine_shadow', '/live-office/static/coffee-machine-shadow-v1.png?v={{VERSION_TIMESTAMP}}');
    this.load.spritesheet('serverroom', '/live-office/static/serverroom-spritesheet.webp?v={{VERSION_TIMESTAMP}}', { frameWidth: 180, frameHeight: 251 });
    // Error / bug animation: 180x180, 96 frames (repacked grid)
    this.load.spritesheet('error_bug', '/live-office/static/error-bug-spritesheet-grid.webp?v={{VERSION_TIMESTAMP}}', { frameWidth: 220, frameHeight: 220 });
    ...
    this.load.spritesheet('cats', '/live-office/static/cats-spritesheet.webp?v={{VERSION_TIMESTAMP}}', { frameWidth: 160, frameHeight: 160 });
    // Desk
    ...
    this.load.spritesheet('star_working', '/live-office/static/star-working-spritesheet-grid.webp?v={{VERSION_TIMESTAMP}}', { frameWidth: 300, frameHeight: 300 });
    // Sync state animation (256x256, 多帧): 非同步显示首帧，同步从第2帧循环
    this.load.spritesheet('sync_anim', '/live-office/static/sync-animation-v3-grid.webp?v={{VERSION_TIMESTAMP}}', { frameWidth: 256, frameHeight: 256 });
    // Memo background image
    // memo 底图固定走 png，避免某些端 webp 透明通道异常导致“底图丢失”
    this.load.image('memo_bg', '/live-office/static/memo-bg.webp?v={{VERSION_TIMESTAMP}}');
    // Desk v2 (webp only)
    this.load.image('desk_v2', '/live-office/static/desk-v3.webp?v={{VERSION_TIMESTAMP}}');
    // Flower spritesheet (65x65, 16 frames)
    this.load.spritesheet('flowers', '/live-office/static/flowers-bloom-v2.webp?v={{VERSION_TIMESTAMP}}', { frameWidth: FLOWERS_FRAME_W, frameHeight: FLOWERS_FRAME_H });
    // Guest/Demo agent sprites
    this.load.spritesheet('guest_anim_1', '/live-office/static/guest_anim_1.webp?v={{VERSION_TIMESTAMP}}', { frameWidth: 32, frameHeight: 32 });
    this.load.spritesheet('guest_anim_2', '/live-office/static/guest_anim_2.webp?v={{VERSION_TIMESTAMP}}', { frameWidth: 32, frameHeight: 32 });
    this.load.spritesheet('guest_anim_3', '/live-office/static/guest_anim_3.webp?v={{VERSION_TIMESTAMP}}', { frameWidth: 32, frameHeight: 32 });
    this.load.spritesheet('guest_anim_4', '/live-office/static/guest_anim_4.webp?v={{VERSION_TIMESTAMP}}', { frameWidth: 32, frameHeight: 32 });
    this.load.spritesheet('guest_anim_5', '/live-office/static/guest_anim_5.webp?v={{VERSION_TIMESTAMP}}', { frameWidth: 32, frameHeight: 32 });
    this.load.spritesheet('guest_anim_6', '/live-office/static/guest_anim_6.webp?v={{VERSION_TIMESTAMP}}', { frameWidth: 32, frameHeight: 32 });
}
function create() {
    game = this;
    hideGameSkeleton();
    ...
    const starIdleFrameMax = Math.max(0, (this.textures.get('star_idle')?.frameTotal || 2) - 2);
    ...
    if (canvasEl) {
        ...
        canvasEl.addEventListener('touchmove', (e) => {
            if (!panEnabled || !touchPan || e.touches.length !== 1)
                return;
            const t = e.touches[0];
    ...
    game.input.on('pointermove', (pointer) => {
        if (!panEnabled || !isPanning || !panStart)
            return;
...
function update(time) {
    ...
    if (syncAnimSprite) {
        if (effectiveStateForServer === 'syncing') {
            ...
            if (syncAnimSprite.anims && syncAnimSprite.anims.isPlaying)
                syncAnimSprite.anims.stop();
            syncAnimSprite.setFrame(0);
        }
    }
...
function showCatBubble() {
    ...
    setTimeout(() => { if (window.catBubble) {
        window.catBubble.destroy();
        window.catBubble = null;
    } }, 4000);
}
